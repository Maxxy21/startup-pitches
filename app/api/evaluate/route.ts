...
// Utility Functions
function parseEvaluationResponse(response: string, aspects: string[]): Evaluation {
...
function buildPrompt(criteriaName: string, aspects: string[], fullContent: string): string {
    return `
    ...
    try {
        ...
        const evaluations: Evaluation[] = await Promise.all(
            Object.entries(EVALUATION_CRITERIA).map(async ([, criteria]) => {
                const prompt = buildPrompt(criteria.name, Array.from(criteria.aspects), fullContent);
                const completion = await makeOpenAIRequest(prompt);
                const response = completion.choices[0].message.content || "";
                const parsed = parseEvaluationResponse(response, Array.from(criteria.aspects));
